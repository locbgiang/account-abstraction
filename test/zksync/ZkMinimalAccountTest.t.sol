...
import {BOOTLOADER_FORMAL_ADDRESS, NONCE_HOLDER_SYSTEM_CONTRACT} from "lib/foundry-era-contracts/src/system-contracts/contracts/Constants.sol";
...
contract ZkMinimalAccountTest is Test {
    ...
    function testZkValidateTransaction () public {
        ...
        // sanity: recovered signer must be owner
        bytes32 txHash = MemoryTransactionHelper.encodeHash(transaction);
        address signer = ECDSA.recover(txHash, transaction.signature);
        assertEq(signer, minimalAccount.owner(), "signature not from owner");
    ...
    function _createUnsignedTransaction(
        ...
        // Use zkSync NonceHolder system contract for the account nonce
        uint256 nonce = INonceHolder(NONCE_HOLDER_SYSTEM_CONTRACT).getMinNonce(address(minimalAccount));
        ...
        return Transaction({
            ...
            // Keep gas-related fields 0 for validation-only flow in tests
            gasLimit: 0,
